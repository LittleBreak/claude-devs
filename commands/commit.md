...
**根据 `$ARGUMENTS` 参数决定暂存策略：**

### 当 `$ARGUMENTS` 包含 `A` 时（不区分大小写）（自动暂存所有变更）：

- 将工作区中的所有变更（包括新增、修改、删除的文件）一次性添加到暂存区
- 但仍需检查是否存在敏感文件（如 `.env`、密钥文件、凭证等），如果有则将其从暂存区移除（`git reset HEAD <file>`）并警告用户

### 当 `$ARGUMENTS` 不包含 `A` 时（默认行为，手动选择暂存）：

- 根据变更内容，将相关文件逐个添加到暂存区
...
## 第五步：推送到远程仓库（仅当 `$ARGUMENTS` 包含 `P` 时）

- 当 `$ARGUMENTS` 包含 `P`（不区分大小写）时，提交成功后自动执行 `git push`
- 如果当前分支没有上游跟踪分支，使用 `git push -u origin <当前分支名>` 设置上游
- 推送完成后确认推送结果

## 注意事项

- 除非 `$ARGUMENTS` 包含 `P` 或用户明确要求，否则禁止自动推送到远程仓库
- 如果没有任何变更，告知用户无需提交
- 如果 pre-commit hook 失败，修复问题后创建**新的提交**，不要 amend
- **禁止**在提交消息中添加 `Co-Authored-By`或类似的署名行
